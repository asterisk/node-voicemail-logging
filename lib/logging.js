...
function create(config, component) {
  ...
  // for backwards compatibility, check that stdout and stderr are not false
  // otherwise default to adding them to the write streams
  if (config.logging.normal.stdout !== false) {
    writeStreams.push({
      level: config.logging.normal.level,
  ...
  if (config.logging.error.stderr !== false) {
    writeStreams.push({
      level: config.logging.error.level,
